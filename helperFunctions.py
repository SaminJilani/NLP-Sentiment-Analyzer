...
def analyze_sentiment_with_context(text):
    ...
    avg_score = sum(score['compound'] for score in sentence_sentiments) / len(sentence_sentiments)
    ...
    confidence = abs(avg_score) * 100  # Confidence is the absolute value of the average score times 100
    confidence = round(confidence, 2)  # Round to two decimal places

    return sentiment, confidence  # Ensure exactly two values are returned
